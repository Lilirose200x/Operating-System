...
int interpreter(char* command_args[], int args_size){
	...
	if (strcmp(command_args[0], "help") == 0) {
		...
		return help();
	} if (0 == strcmp(command_args[0], "resetmem")) {
		if (args_size != 1) 
			return badcommand();
		return resetmem();
...
int exec(char *fname1, char *fname2, char *fname3, char* policy){
	int error_code;
	int policyNumber;
	printf("Frame Store Size = %d; Variable Store Size = %d\n", FRAME_MEM_LENGTH, VAR_MEM_LENGTH);
    error_code = 0;

	policyNumber = get_scheduling_policy_number(policy);
	if(policyNumber == 15){
...
//	if(strcmp(mem_get_value_by_line(i), "none") != 0){
//	printf("%s\n", mem_get_value_by_line(i));}
//}
